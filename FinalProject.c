...
void showOrders() {
    ...
    printf("Press C to checkout or any other key to go back: ");
    scanf("%s", choice);
...
void processCheckout() {
    ...
    for (int i = 0; i < orderCount; i++) {
        ...
        char upperType = toupper(types[i][0]);
        double itemPrice = (upperType == 'S') ? specialPrices[itemIndex] : regularPrices[itemIndex];
        double lineTotal = itemPrice * quantities[i];
